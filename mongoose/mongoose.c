...
void listdir(struct mg_connection *c, struct mg_http_message *hm,
...
int uri_to_path(struct mg_connection *c, struct mg_http_message *hm,
...
size_t mg_ws_wrap(struct mg_connection *c, size_t len, int op) {
 ...
 void printdirentry_directory_json(const char* name, void* userdata) {
    struct printdirentrydata_json* d = (struct printdirentrydata_json*)userdata;
    struct mg_fs* fs = d->opts->fs == NULL ? &mg_fs_posix : d->opts->fs;
    size_t size = 0;
    time_t t = 0;
    char path[MG_PATH_MAX], sz[40], mod[40];
    ...
    if (mg_snprintf(path, sizeof(path), "%s%c%s", d->dir, '/', name) > sizeof(path)) {
        ...
        if (flags & MG_FS_DIR)
        {
            mg_printf(d->c, "{\"name\":\"%s\", \"modified\":%d},\n", name, t);
            d->total_entitys += 1;
        }
    }
}

void printdirentry_files_json(const char* name, void* userdata) {
    struct printdirentrydata_json* d = (struct printdirentrydata_json*)userdata;
    struct mg_fs* fs = d->opts->fs == NULL ? &mg_fs_posix : d->opts->fs;
    size_t size = 0;
    time_t t = 0;
    ...
    if (mg_snprintf(path, sizeof(path), "%s%c%s", d->dir, '/', name) > sizeof(path)) {
        ...
        if (!(flags & MG_FS_DIR))
        {
            mg_printf(d->c, "{\"name\":\"%s\", \"size\":%llu, \"modified\":%d},\n", name, size, t);
            d->total_entitys += 1;
        }
    }
}

void listdir_json(struct mg_connection* c, struct mg_http_message* hm,
    const struct mg_http_serve_opts* opts, char* dir) {
    struct mg_fs* fs = opts->fs == NULL ? &mg_fs_posix : opts->fs;
    struct printdirentrydata_json d = { c, hm, opts, dir, 0 };
    char tmp[10], buf[MG_PATH_MAX];
    size_t off, n;
    int len = mg_url_decode(hm->uri.ptr, hm->uri.len, buf, sizeof(buf), 0);
    struct mg_str uri = len > 0 ? mg_str_n(buf, (size_t)len) : hm->uri;
    ...
    mg_printf(c, "\n\"dir\":[\n");
    d.total_entitys = 0;
    fs->ls(dir, printdirentry_directory_json, &d);
    if (d.total_entitys > 0) {
        c->send.len -= 2;
    }
    mg_printf(c, "\n],");
    mg_printf(c, "\n\"file\":[\n");
    d.total_entitys = 0;
    fs->ls(dir, printdirentry_files_json, &d);
    if (d.total_entitys > 0) {
        c->send.len -= 2;
    }
    mg_printf(c, "\n]");
...
void mg_http_serve_dir_json(struct mg_connection* c, struct mg_http_message* hm,
    const struct mg_http_serve_opts* opts) {
    char path[MG_PATH_MAX];
    const char* sp = opts->ssi_pattern;
    int flags = uri_to_path(c, hm, opts, path, sizeof(path));
...
void mg_http_dir_list_json(struct mg_connection* c, struct mg_http_message* hm,
    const struct mg_http_serve_opts* opts, char* dir) {
    char *path = dir;
    char* temp_Path;
    const char* sp = opts->ssi_pattern;
    int flags = fileStat(opts, dir);
    ...
    if (flags <= 0) {
        ...
        if (fd == NULL && opts->page404 != NULL) {
            fd = mg_fs_open(fs, opts->page404, MG_FS_READ);
            temp_Path = opts->page404;
        }

        if (fd == NULL || fs->st(temp_Path, &size, &mtime) == 0) {
            mg_http_reply(c, 404, opts->extra_headers, "Not found\n");
            mg_fs_close(fd);
            // NOTE: mg_http_etag() call should go first!
        }
...
int fileStat(struct mg_http_serve_opts* opts, char* path) {
    struct mg_fs* fs = opts->fs == NULL ? &mg_fs_posix : opts->fs;
    int flags;
    //int n = strlen(path);
    if (!mg_path_is_sane(path)) {
    ...
    //while (n > 1 && path[n - 1] == '/') path[--n] = 0;  // Trim trailing slashes
    flags = fs->st(path, NULL, NULL);

    return flags;
}

bool mg_mkdir(struct mg_http_serve_opts* opts, char* path) {
    struct mg_fs* fs = opts->fs == NULL ? &mg_fs_posix : opts->fs;
    int n = strlen(path);
    if (!mg_path_is_sane(path)) {
        return false;
...
void rmdir_r(const char* name, void* userdata) {
    struct rmDirectory* ctx;
    char* path_end;
    char* path;
    int flags;

    ctx = (struct rmDirectory*)userdata;
    if (ctx->error != 0) {
        return;
    }
    ...
    path_end = ctx->path_end;
    path = ctx->path;

    strcpy(ctx->path_end, name);
    ctx->path_end += strlen(name);
    flags = ctx->fs->st(path, NULL, NULL);
    ...
    if (flags == 0) {
        ...
        ctx->fs->ls(path, rmdir_r, (void*)ctx);

        if (ctx->error != 0) {
            return;
        }

        if (ctx->fs->rm(path) == false) {
            ctx->error = 1;
        }
    }
    else {
...
bool mg_rmdir(struct mg_http_serve_opts* opts, char* path) {
    struct mg_fs* fs = opts->fs == NULL ? &mg_fs_posix : opts->fs;
    struct rmDirectory ctx = {
        .fs = fs,
        .error = 0,
    ...
    if (fileStat(opts, path) & MG_FS_DIR) {
        fs->ls(path, rmdir_r, (void*)&ctx);
        if (ctx.error != 0) {
            return false;
        }
        return fs->rm(path);
...
bool mg_rm(struct mg_http_serve_opts* opts, char* path) {
    struct mg_fs* fs = opts->fs == NULL ? &mg_fs_posix : opts->fs;
    int n = strlen(path);
    if (!mg_path_is_sane(path)) {
        return false;
